...
public class Key extends SuperObject{
	...
	public void update() {
		...
		else if(this.speedY > 0) this.directionY = "down";
		...
		if(gp.player != null) {
			if(this.inAir == true) {
				if(gp.player.worldY >= this.worldY
						&& gp.player.worldY <= this.worldY + 16) {
					if(gp.player.worldX >= this.worldX - 40
		...
		for(int i=0; i<5; i++) {
			if(gp.clone[i] != null) {
				if(this.inAir == true) {
					if(gp.clone[i].worldY >= this.worldY
							&& gp.clone[i].worldY <= this.worldY + 16) {
						if(gp.clone[i].worldX >= this.worldX - 40
								&& gp.clone[i].worldX <= this.worldX + 40) {
		...
		for(int i=0; i<=1; i++) {
			if(gp.obj[i] != null && i != gp.sttobj && this.inAir == false) {
				if(gp.obj[i].worldY >= this.worldY - 40
						&& gp.obj[i].worldY <= this.worldY + 40) {
//				if(gp.obj[i].worldY == gp.obj[k].worldY ) {
					if(gp.obj[i].worldX >= this.worldX
							&& gp.obj[i].worldX <= this.worldX + 48) {
						gp.obj[i].worldX = this.worldX + 48;
					}
					else if(gp.obj[i].worldX <= this.worldX
							&& gp.obj[i].worldX >= this.worldX - 48) {
						gp.obj[i].worldX = this.worldX - 48;
					}	
				}
